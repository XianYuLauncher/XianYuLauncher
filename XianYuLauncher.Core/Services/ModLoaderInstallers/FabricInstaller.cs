...
public class FabricInstaller : ModLoaderInstallerBase
{
    ...
    public override async Task<string> InstallAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        Logger.LogInformation("开始安装 Fabric: {FabricVersion} for Minecraft {MinecraftVersion}, SkipJarDownload={SkipJar}",
        ...
        try
        {
            ...
            // 3. 获取 Fabric Profile
            Logger.LogInformation("获取 Fabric Profile");
            ...
            // 6. 下载 Fabric 库文件
            Logger.LogInformation("下载 Fabric 库文件");
            ...
            // 7. 生成 Fabric 版本 JSON（与原版合并）
            Logger.LogInformation("生成 Fabric 版本 JSON");
            ...
            Logger.LogInformation("Fabric 安装完成: {VersionId}", versionId);
            return versionId;
        }
        catch (OperationCanceledException)
        {
            Logger.LogWarning("Fabric 安装已取消");
            throw;
        }
        catch (Exception ex) when (ex is not ModLoaderInstallException)
        {
            Logger.LogError(ex, "Fabric 安装失败");
            throw new ModLoaderInstallException(
                $"Fabric 安装失败: {ex.Message}",
                ModLoaderType,
                modLoaderVersion,
                minecraftVersionId,
                innerException: ex);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            Logger.LogError(ex, "获取 Fabric 版本列表失败: {MinecraftVersion}", minecraftVersionId);
        ...
        CancellationToken cancellationToken)
    {
        ...
        Logger.LogInformation("使用下载源 {DownloadSource} 获取 Fabric Profile: {Url}", downloadSource.Name, url);
    ...
    private VersionInfo ResolveVersionInfo(VersionInfo original, JObject fabricProfile, string versionId)
    {
        ...
        Logger.LogInformation("通过 ManifestPatch 解析了 {LibraryCount} 个 Fabric 依赖库", manifestPatch.Libraries?.Count ?? 0);
