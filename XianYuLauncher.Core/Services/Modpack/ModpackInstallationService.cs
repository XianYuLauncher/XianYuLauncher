...
public class ModpackInstallationService : IModpackInstallationService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString());
            string modpackPath = Path.Combine(tempDir, fileName);
            Directory.CreateDirectory(tempDir);
            ...
            if (File.Exists(curseForgeManifestPath))
            {
                return await UpdateCurseForgeModpackInPlaceAsync(
                    extractDir,
            ...
            if (File.Exists(modrinthIndexPath))
            {
                return await UpdateModrinthModpackInPlaceAsync(
                    extractDir,
                    modrinthIndexPath,
                    modpackDisplayName,
                    minecraftPath,
    ...
    private async Task<ModpackInstallResult> UpdateModrinthModpackInPlaceAsync(
        string extractDir,
        string indexPath,
        string modpackDisplayName,
        string minecraftPath,
        ...
        CancellationToken cancellationToken)
    {
        ...
        ReportQueuedContentFiles(
            contentFileProgress,
            downloadEntries.Select(file => (file.FileKey, file.FileDisplayName)));
    ...
    private async Task<ModpackInstallResult> UpdateCurseForgeModpackInPlaceAsync(
        string extractDir,
        string manifestPath,
        string modpackDisplayName,
        string minecraftPath,
    ...
    {
        return PathContainmentHelper.GetValidatedPathUnderRoot(rootDirectory, relativePath, pathDescription);
    }

    internal static string SanitizeExternalFileName(string? fileName, string fallbackFileName, string fileDescription)
    {
        return PathContainmentHelper.SanitizeExternalFileName(fileName, fallbackFileName, fileDescription);
    }
    ...
    private async Task<int> ResolveContentDownloadParallelismAsync(CancellationToken cancellationToken)
    {
    ...
    private static void ReportQueuedContentFiles(
        IProgress<ModpackContentFileProgress>? contentFileProgress,
        IEnumerable<(string FileKey, string FileDisplayName)> files)
    {
        if (contentFileProgress == null)
        {
            return;
        }
    ...
    private static void CleanManagedDirectories(string versionDirectory)
    {
        var managedDirs = new[]
        {
            "mods",
        ...
        {
            var fullPath = Path.Combine(versionDirectory, dir);
