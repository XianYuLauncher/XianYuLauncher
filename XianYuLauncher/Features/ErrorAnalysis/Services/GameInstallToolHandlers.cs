...
public sealed class GetOperationStatusToolHandler : IAgentToolHandler
{
    ...
    public AiToolDefinition ToolDefinition => AiToolDefinition.Create(
        ToolName,
        "根据 operation_id 查询 installGame、installCommunityResource、updateInstanceCommunityResources 或 launchGame 返回的任务状态。",
        new
        {
            type = "object",
            properties = new
            {
                operation_id = new { type = "string", description = "installGame、installCommunityResource、updateInstanceCommunityResources 或 launchGame 执行后返回的 operation_id" }
            },
