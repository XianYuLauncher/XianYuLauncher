...
public class ErrorAnalysisAIOrchestrator : IErrorAnalysisAIOrchestrator
{
    ...
    private static string BuildDefaultFunctionCallingSystemPrompt(string language)
    {
        return
            ...
            "4. For instance-local community resource management, prefer 'getInstances' -> 'getInstanceCommunityResources' -> 'checkInstanceCommunityResourceUpdates' -> 'updateInstanceCommunityResources' -> 'getOperationStatus'. For existing modpack instances, prefer 'getInstances' or 'getVersionConfig' to confirm is_modpack and modpack metadata first, then use 'checkModpackUpdate' -> 'updateModpack' -> 'getOperationStatus'. If the user wants to search remote resources by category, call 'getCommunityResourceTags' first, then pass categories[*].token into 'searchCommunityResources.category_tokens'. If the user wants to browse what is currently popular, you may call 'searchCommunityResources' without query. When the user wants to inspect a remote resource's detailed categories, loaders, compatibility, or description before deciding, call 'getCommunityResourceProjectDetail'. Continue with 'getCommunityResourceFiles' -> 'installCommunityResource' only after the project choice is clear for mod / resourcepack / shader / world. For world installs, confirm the target instance explicitly via target_version_name or target_version_path before execution. For modpacks, continue with 'getCommunityResourceFiles' -> 'installModpack'; its target_version_name must be a NEW instance name instead of an existing instance. Only use 'searchModrinthProject' when the goal is to open the UI detail page.\n" +
