...
public class ErrorAnalysisAIOrchestrator : IErrorAnalysisAIOrchestrator
{
    ...
    private static string BuildDefaultFunctionCallingSystemPrompt(string language)
    {
        return
            ...
            "4. For instance-local community resource management, prefer 'getInstances' -> 'getInstanceCommunityResources' -> 'checkInstanceCommunityResourceUpdates' -> 'updateInstanceCommunityResources' -> 'getOperationStatus'. When the user wants to search remote resources by category, call 'getCommunityResourceTags' first, then pass categories[*].token into 'searchCommunityResources.category_tokens'. When the user wants to inspect a remote resource's detailed categories, loaders, compatibility, or description before deciding, call 'getCommunityResourceProjectDetail'. Continue with 'getCommunityResourceFiles' -> 'installCommunityResource' only after the project choice is clear. Only use 'searchModrinthProject' when the goal is to open the UI detail page.\n" +
